...
var userId = "Tyler-Seliber";
var config = {};
var currentRepo = "SSW-345"
var newrepo = "Created-From-REST";
var issue = "REST Issue"
...
if (process.env.NODE_ENV != 'test')
{
	(async () => {
		...
		await listBranches(userId, currentRepo);
		await createRepo(userId, newrepo);
		await createIssue(userId, newrepo, issue);
		await enableWikiSupport(userId, newrepo);
...
async function createRepo(owner,repo)
{
	...
	options.json = {
		"name": repo,
		"has_wiki": false // disable wiki support by default so enableWikiSupport can be used
	};
